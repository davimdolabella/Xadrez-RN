...
export default (col_or_lin = "lin" , up_or_down = "down",peca_detail, tabuleiro ,is_valid , pecas )=>{
    let posicpossiveis = []
    ...
    if(col_or_lin === "lin" && is_valid(lin) && (is_valid(col - 1) || is_valid(col+ 1))){ 
        let peca1 =  is_valid(col + 1)? tabuleiro[lin][col+1]: [null,peca_detail.cor]
        let peca2 = is_valid(col - 1)? tabuleiro[lin][col-1]: [null,peca_detail.cor]
        ...
        let peca1 =  is_valid(lin - 1)? tabuleiro[lin-1][col] : [null,peca_detail.cor]
        let peca2 = is_valid(lin + 1)? tabuleiro[lin+1][col] : [null,peca_detail.cor]
